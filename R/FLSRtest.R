...
setGeneric("srrTMB1", function(object, ...) standardGeneric("srrTMB1"))

#' @rdname srrTMB

setMethod("srrTMB1", signature(object="FLSRs"),
  function(object, ...) {
...
setMethod("srrTMB1", signature(object="FLSR"),
  function(object, spr0="missing",
  s=NULL, s.est=TRUE, s.logitsd=20, r0.pr="missing",
  lplim=0.001, uplim=0.3, Blim="missing",
  ...
  if(is.null(lplim)){
    if(is.null(s)& s.est){ lplim=0.01} else {lplim=0.1}
  }
  ...
  if(!model=="mean"){
    ...
    if(model=="segreg"){
      ...
      srp = max(min(srp,0.9*uplim,srp),lplim*1.1)
      if(is.null(s.inp)){
        sinit = 0.99 #1/1.1#1/(srp/lplim)
      } else {
        sinit =  s.inp
      }
    ...
    inp <- list(
      ...
      Data = list(ssb = ssb, rec = rec,prior_s = c(to_logits(s,ll,ul),s.logitsd), prior_r0 = prior_r0,
                  spr0y = spr0.yr,spr0=spr0ref,plim=lplim, nyears=length(ssb),slim=ll,smax=ul,
